...
func AddRepository(repo models.RepoRecord) error {
	...
	sql := "insert into repository (owner_id, project_id, name, description, pull_count, star_count, creation_time, update_time) " +
		"select (select user_id as owner_id from tenx_users where user_name=?), " +
...
func GetAllRepositories() ([]models.RepoRecord, error) {
	// Should just sort based on project name, or / will be used when compare as the repository name
	sql := `select * from repository order by substring_index(name,'/',1)`
	var repos []models.RepoRecord
	_, err := GetOrmer().Raw(sql).QueryRows(&repos)
	return repos, err
}
		...
		where (
			p.deleted = 0 and (
				p.public = 1 or (
					? <> ? and (
						exists (
							select 1 from tenx_users u
							where u.user_id = ? and u.admin_role = 1
						) or exists (
