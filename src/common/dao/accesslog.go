...
func GetTotalOfAccessLogs(query models.AccessLog) (int64, error) {
	...
	if query.Username != "" {
		sql = `select count(*) from access_log al 
			left join tenx_users u 
			on al.user_id = u.user_id 
			where al.project_id = ? and u.user_name like ? `
...
func GetAccessLogs(query models.AccessLog, limit, offset int64) ([]models.AccessLog, error) {
	...
	queryParam := []interface{}{}
	sql := `select al.log_id, u.user_name as username, al.repo_name, 
...
func genFilterClauses(query models.AccessLog, queryParam *[]interface{}) string {
	...
	if query.RepoName != "" {
		sql += ` and al.repo_name like ? `
		*queryParam = append(*queryParam, "%"+escape(query.RepoName)+"%")
	}
	if query.RepoTag != "" {
		sql += ` and al.repo_tag = ? `
...
func AccessLog(username, projectName, repoName, repoTag, action string) error {
	...
	sql := "insert into  access_log (user_id, project_id, repo_name, repo_tag, operation, op_time) " +
		"select (select user_id as user_id from tenx_users where user_name=?), " +
...
func GetRecentLogs(userID, linesNum int, startTime, endTime string) ([]models.AccessLog, error) {
	...
	queryParam := []interface{}{}
	sql := `select log_id, access_log.user_id, project_id, repo_name, repo_tag, GUID, operation, op_time, user_name 
		from access_log 
		join tenx_users 
		on access_log.user_id=tenx_users.user_id `
...
func GetAccessLogCreator(repoName string) (string, error) {
	o := GetOrmer()
	sql := "select * from tenx_users where user_id = (select user_id from access_log where operation = 'push' and repo_name = ? order by op_time desc limit 1)"
