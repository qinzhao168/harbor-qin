...
func GetProjectByID(id int64) (*models.Project, error) {
	...
	sql := `select p.project_id, p.name, u.user_name as owner_name, p.owner_id, p.creation_time, p.update_time, p.public  
		from project p left join tenx_users u on p.owner_id = u.user_id where p.deleted = 0 and p.project_id = ?`
...
func GetPermission(username, projectName string) (string, error) {
	...
	sql := `select r.role_code from role as r
		...
		inner join tenx_users as u on u.user_id = pm.user_id
		inner join project p on p.project_id = pm.project_id
		where u.user_name = ? and p.name = ? and u.migrated = 0 and p.deleted = 0`
